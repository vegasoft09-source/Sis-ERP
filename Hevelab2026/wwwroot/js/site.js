...
(function () {
  ...
  function init() {
    ...
    // Menús desplegables del sidebar (Compras, Contabilidad, Inventario)
    function setupNavSubmenu(btnId, menuId, iconId) {
    ...
    var toggleMenuCompras = setupNavSubmenu('btnMenuCompras', 'menuCompras', 'iconMenuCompras');
    var toggleMenuContabilidad = setupNavSubmenu('btnMenuContabilidad', 'menuContabilidad', 'iconMenuContabilidad');
    var toggleMenuInventario = setupNavSubmenu('btnMenuInventario', 'menuInventario', 'iconMenuInventario');

    var currentPath = window.location.pathname.toLowerCase();

    if (currentPath.includes('/solicitud') ||
        currentPath.includes('/ordenes') ||
        currentPath.includes('/proveedores')) {
      if (toggleMenuCompras) toggleMenuCompras(true);
    }

    if (currentPath.includes('/facturas') ||
        currentPath.includes('/libromayor') ||
        currentPath.includes('/sire') ||
        currentPath.includes('/documentos')) {
      if (toggleMenuContabilidad) toggleMenuContabilidad(true);
    }

    if (currentPath.includes('/recepciones') ||
        currentPath.includes('/producto')) {
      if (toggleMenuInventario) toggleMenuInventario(true);
    }
